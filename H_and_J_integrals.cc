...
//        cout << "M = " << M << endl;
        ...
        while(outside_error > epsilon/2) {
            ...
            while(inside_error *  outside_error > epsilon/6) {
...
//        cout << "M = " << M << endl;
        ...
        while(L_power1 * outside_error > epsilon/2) {
            Double a_minus_a0_power = 1.0;
            ...
            while(inside_error *  outside_error * L_power2 > epsilon/6) {
                //Complex z = minus_one_power(l) * two_pi_over_factorial_power(l) * a_minus_a0_power * F0_cache.values[M][a0_index][b0_index][j + l + 2 * s];
                //Complex z = minus_one_power(l) * two_pi_over_factorial_power(l) * a_minus_a0_power * get_cached_F0_value(a0_index, b0_index, j + l + 2 * s, M);
                Complex z = minus_one_power(l) * two_pi_over_factorial_power(l) * a_minus_a0_power;
                inside_error = abs(z);
...
Complex J_Integral_2(Double a1, Double a2, Double b, int j, int K, theta_cache * cache, Double epsilon, bool use_cache) {
    //
    ...
    if(cache) {
        ...
        K_pow_minus_j = pow(K, -j);
        K_pow_j = 1.0/K_pow_minus_j;
    }

    if(a2 > a1) {
        //if (2.0 * pow(K, -j) * ( (a2 - a1)/a2 * log(1 + a2) ) < epsilon)
        if(1 + fastlog2(K_pow_minus_j) + fastlog2( (a2 - a1)/a2 * (fastlog(1 + a2) + 1.0)) < fastlog2(epsilon))
            return 0.0;
    }
    else {
        //if (2.0 * pow(K, -j) * ( (a1 - a2)/a1 * log(1 + a1) ) < epsilon)
        if(1 + fastlog2(K_pow_minus_j) + fastlog2( (a1 - a2)/a1 * (fastlog(1 + a1) + 1.0)) < fastlog2(epsilon))
            return 0.0;
    }
    ...
    if(b == 0) {
        return H_Integral_2(j, a1, a2, epsilon) * K_pow_minus_j;
    }
...
//            cout << "b index = " << l << ", b = " << b << endl;
            ...
            for(long j = 0; j <= max_j; j++) {
                ...
                F1_cache.values[k][l][j] = J_Integral_1(a, b, j, -1, 1, NULL, epsilon, false);
            }
            b += b_increment;
        }
        a += a_increment;
...
void build_F2_cache(long max_a1, long number_of_a1, long number_of_a2, long number_of_b, Double epsilon) {
    ...
    Double a1_increment = max_a1/(number_of_a1 - 1.0);
...
//            cout << a2 << endl;
            ...
            for(long l = 0; l < number_of_b; l++) {
                F2_cache.values[k][j][l] = J_Integral_2(a1, a2, b, 0, 1, NULL, epsilon, false);
                b += b_increment;
            }
            a2 += a2_increment;
            b = 0.0;
